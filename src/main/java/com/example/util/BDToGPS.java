...
public class BDToGPS {
	...
	public static void main(String[] args){
		/*double[] ll = gcj2WGSExactly(23.37896887976603,116.69869566268229);
		System.out.println(ll[0]+","+ll[1]);
		double[] ll2 = gcj2WGS(23.37896887976603,116.69869566268229);
		System.out.println(ll2[0]+","+ll2[1]);*/
		System.out.println("百度坐标:23.25499,116.412736");
		//百度坐标转火星坐标
		double[] ll = bd092GCJ(23.25499,116.412736);
		System.out.println("火星坐标:"+ll[0]+","+ll[1]);
		//百度坐标转gps坐标
		double[] ll2 = bd092WGS(23.25499,116.412736);
		System.out.println("gps坐标:"+ll2[0]+","+ll2[1]);
	}
	 ...
	 * @return
	 */
	public static String gpsToGaode (String string){
		String[] array = string.split(";");
		String result = "";
		for(int i=0;i<array.length;i++){
			String[] lnglat = array[i].split(",");
			double[] gaode = wgs2GCJ(Double.valueOf(lnglat[1]), Double.valueOf(lnglat[0]));
			result = result + gaode[1] + "," + gaode[0];
			if (i<array.length-1) {
				result = result + ";";
			}
		}
	 ...
	 * @return
	 */
	public static String gpsToBaidu (String string){
		String[] array = string.split(";");
		String result = "";
		for(int i=0;i<array.length;i++){
			String[] lnglat = array[i].split(",");
			double[] gaode = wgs2BD09(Double.valueOf(lnglat[1]), Double.valueOf(lnglat[0]));
			result = result + gaode[1] + "," + gaode[0];
			if (i<array.length-1) {
				result = result + ";";
			}
	 ...
	 * @return
	 */
	public static String gaodeToGps (String string){
		String[] array = string.split(";");
		String result = "";
		for(int i=0;i<array.length;i++){
			String[] lnglat = array[i].split(",");
			double[] gaode = gcj2WGSExactly(Double.valueOf(lnglat[1]), Double.valueOf(lnglat[0]));
			result = result + gaode[1] + "," + gaode[0];
			if (i<array.length-1) {
				result = result + ";";
			}
	 ...
	 * @return
	 */
	public static String gaodeToBaidu (String string){
		String[] array = string.split(";");
		String result = "";
		for(int i=0;i<array.length;i++){
			String[] lnglat = array[i].split(",");
			double[] gaode = gcj2BD09(Double.valueOf(lnglat[1]), Double.valueOf(lnglat[0]));
			result = result + gaode[1] + "," + gaode[0];
			if (i<array.length-1) {
				result = result + ";";
			}
